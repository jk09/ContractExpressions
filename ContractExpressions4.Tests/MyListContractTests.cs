...
public class MyListContractTests : IClassFixture<ContractFailureUnwindFixture>
{
    ...
    public void Indexer_Get_WithNegativeIndex_ThrowsException()
    {
        var item = new object();
        _proxy.Add(item);
        var ex = Assert.Throws<ContractViolationException>(() => _proxy[-1]);
    ...
    {
        var ex = Assert.Throws<ContractViolationException>(() => _proxy.Insert(-1, new object()));
        Assert.Equal(ContractKind.Precondition, ex.Kind);
    }
    ...
    {
        _proxy.Add(new object());
        var ex = Assert.Throws<ContractViolationException>(() => _proxy.Insert(2, new object()));
        Assert.Equal(ContractKind.Precondition, ex.Kind);
    }
    ...
    {
        _proxy.Add(new object());
        var ex = Assert.Throws<ContractViolationException>(() => _proxy.RemoveAt(-1));
        Assert.Equal(ContractKind.Precondition, ex.Kind);
    }
    ...
    public void CopyTo_WithNegativeArrayIndex_ThrowsException()
    {
        _proxy.Add(new object());
        var array = new object[5];
        var ex = Assert.Throws<ContractViolationException>(() => _proxy.CopyTo(array, -1));
        Assert.Equal(ContractKind.Precondition, ex.Kind);
    }
