...
public class BankAccountContractTests : IClassFixture<ContractFailureUnwindFixture>
{
    ...
    {
        var ex = Assert.Throws<ContractViolationException>(() => _proxy.Deposit(0f));
    ...
    {
        var ex = Assert.Throws<ContractViolationException>(() => _proxy.Deposit(-10f));
        Assert.Equal(ContractKind.Precondition, ex.Kind);
    }
    ...
    {
        var ex = Assert.Throws<ContractViolationException>(() => _proxy.Withdraw(0f));
        Assert.Equal(ContractKind.Precondition, ex.Kind);
    }
    ...
    public void Withdraw_ExceedsBalancePlusOverdraft_ThrowsPreconditionFailure()
    {
        ...
        var ex = Assert.Throws<ContractViolationException>(() => _proxy.Withdraw(151f));
        Assert.Equal(ContractKind.Precondition, ex.Kind);
    }
